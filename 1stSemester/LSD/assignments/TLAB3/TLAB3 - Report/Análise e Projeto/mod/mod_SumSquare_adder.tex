\begin{tikzpicture}[thick,node distance=4cm]
    ...
    % Clip
    \clip ([yshift=2.1cm]reset.west) rectangle ([shift={(0,0)}]mux.south);
    ...
    \node[io, label={[xshift=.2cm]above:A},xshift=-.625cm,label={[scale=.8]below:(8 bits)}] (a) at (adder.north) {};
    ...
    % Outputs
    \node[output,xshift=4cm,minimum width = 2cm] (out) at (reg.east) {Out};
    ...
    % Connections
    \draw (d.west) -| (r_a.south) node[label={[scale=.8,yshift=-.3cm]left:(8 bits)}] {};
