\begin{tikzpicture}[thick,node distance=4cm]
    \begin{scope}[scale=0.8, transform shape]
        ...
        % Clip
        \clip ([shift={(-.1cm,-2cm)}]reset.west) rectangle ([shift={(1cm,-.5cm)}]cout.south);

        % Register block
        \node[module,xshift=-1cm] (reg) {Registo};
        \node[io, label={[yshift=-.2cm]left:D},yshift=.625cm,label={[scale=.8]right:(8 bits)}] (d) at (reg.west) {};
        \node[io, label={[yshift=-.2cm]left:Enable},yshift=-.625cm] (en) at (reg.west) {};
        \node[io, label={[yshift=-.2cm]left:Reset}] (rst) at (reg.west) {};

        \node[io, label={[yshift=.2cm]right:Q}] (Q) at (reg.east) {};
        ...
        \node[input,xshift=-2.5cm,minimum width = .5cm, minimum height = .5cm] (reset) at (rst.west) {Reset};
        \node[io] (reset_o) at (reset.east) {};

        % Outputs
        \node[output,xshift=3cm,minimum width = 2cm,label={[scale=.8]above:(8 bits)}] (out) at (reg.east) {Out};
        \node[io] (out_o) at (out.west) {};
        \node[io,xshift=1cm] (node) at (reg.east) {};
        ...
        \draw (b_m.north) --++(0,.5cm) --++(1cm,0) -| (node.north);
        \draw (node.west) -- (Q.east);
        ...
        \draw (node) node[label={[scale=.8]below:(8 bits)}] {}  -- (out_o.west);
        \draw (cout_adder.south) |- (cout_o.east);
        \draw (en.west) --++(-1.5cm,0) --++(0,-1cm) --++(5.6cm,0) --++(0,6cm);
